...
export async function startSocketServer(port) {
	...
	var ATC = { username: "", socket: undefined } // all ATCs
	...
	io.on("connection", (socket) => {
		...
		socket.on("DISPATCH", async (flightid) => {
			...
			for (const passenger of flightInfo.passengers) {
				const psocket = passengerMap.get(passenger.username);
				if (psocket) {
					psocket.emit("BOARDING_CALL", flightid);
			...
			setTimeout(() => {
				...
				const handle = setInterval(() => {
					...
					progress = Math.min((Date.now() - startTime) / (parseFloat(flightInfo.flight.flight_duration_hours) * 1000), 1.0);
		...
		socket.on("BOARD", async (flightid) => {
			if (boardingCallSet.has(flightid)) {
				...
				if (ATC.socket) {
					ATC.socket.emit("UPDATEBOARDSTATUS", flightid, socket.data.username);
				}
			}
		...
		socket.on("TRACK", async (flightid) => {
			...
			const handle = setInterval(async () => {
				...
				let flightData = await getFlight(flightid, socket.data.api_key);
				socket.emit("POSITION", flightData.data.flight.current_latitude, flightData.data.flight.current_longitude, flightData.data.flight.status);
				if (flightData.data.flight.status == "Landed") {
					clearInterval(handle);
